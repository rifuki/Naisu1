...
// Route config
export const Route = createFileRoute("/test-claim-fees")({
    component: TestClaimFees,
});
...
export default function TestClaimFees() {
        ...
        try {
            ...
            for (const obj of objects.data) {
                if (obj.data?.content?.dataType === "moveObject") {
                    ...
                    // Only show positions with liquidity > 0
                    const liquidity = fields.liquidity || "0";
                    if (BigInt(liquidity) > 0) {
                        positionList.push({
            ...
            addLog(`✅ Loaded ${positionList.length} positions with liquidity`);

            if (positionList.length === 0) {
                addLog("ℹ️ No positions with liquidity found.");
            }
        } catch (e: any) {
    ...
    const claimFees = async () => {
        ...
        // Find the selected position to get its pool
        ...
        setIsLoading(true);
        setTxResult(null);
        addLog(`💰 Claiming fees from position ${selectedPosition.slice(0, 10)}...`);

        try {
            const tx = new Transaction();

            // Create empty coins to receive fees
            addLog("🔧 Creating empty coins to receive fees...");
            ...
            // Call collect_fee
            addLog("🔧 Calling pool_script_v3::collect_fee...");
            tx.moveCall({
            ...
            signAndExecuteTransaction(
                ...
                {
                    onSuccess: (result) => {
                        addLog(`✅ Fees claimed! Digest: ${result.digest}`);
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-amber-900/20 to-gray-900 p-8">
            <div className="max-w-lg mx-auto">
                {/* Header */}
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-bold text-white mb-2">💰 Claim Fees</h1>
                    <p className="text-amber-300">Collect LP Rewards from Cetus</p>
                </div>

                {/* Main Card */}
                <div className="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 border border-amber-500/20">
                    {/* Pool Info */}
                    <div className="mb-6 p-4 bg-amber-500/10 rounded-xl">
                        <div className="flex justify-between items-center">
                            <span className="text-amber-300">Pool</span>
                            <span className="text-white font-bold">{POOL.name}</span>
                        </div>
                        <p className="text-xs text-amber-300/60 mt-2">
                            Fees are earned when traders swap through this pool while your liquidity is in range.
                        </p>
                    </div>
                    ...
                    {positions.length > 0 && (
                        <div className="mb-6">
                            <label className="block text-amber-300 text-sm mb-2">Select Position to Claim Fees</label>
                                ...
                                {positions.map((pos) => (
                                    <div
                                        ...
                                        className={`p-3 rounded-xl cursor-pointer transition ${selectedPosition === pos.id
                                                ...
                                                : "bg-gray-700/50 border border-gray-600 hover:border-amber-400"
                                            }`}
                                    >
                                        <div className="flex justify-between items-center">
                    ...
                    {positions.length === 0 && !isLoading && (
                        <div className="mb-6 p-4 bg-gray-700/30 rounded-xl text-center">
                            <p className="text-gray-400">No positions with liquidity found</p>
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-gradient-to-r from-amber-500 to-yellow-500 hover:from-amber-400 hover:to-yellow-400 disabled:opacity-50 text-gray-900 rounded-xl font-bold text-lg transition"
                    >
                    ...
                    <p className="text-xs text-gray-400 text-center mt-3">
                        ℹ️ Fees are claimed in both {POOL.coinASymbol} and {POOL.coinBSymbol}
                    </p>
                    ...
                    {txResult && (
                        <div className="mt-4 p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl">
                            <p className="text-emerald-400 font-semibold">✅ Fees Claimed!</p>
                            <a
